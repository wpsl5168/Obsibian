...
> 05-个人、00-收件箱、50-日报与动态及90-治理采用各自执行/流水规范，不强套知识文章格式。
...
**Wiki外**（自由区）：
- `00-收件箱/` — 草稿、待整理（由收件箱索引，不按知识正文lint）
- `05-个人/` — 本人家庭、旅行与个人执行资料
- `50-日报与动态/` — 时序记录（按主题流/月分组索引，不按知识正文lint）
- `90-治理/` — 元数据（迁移记录、周报）
...
| 页面超过200行 | **检查阅读导航**；可独立维护的章节才提议拆页，拆正文另获授权 |
| 内容已被新页面完全替代 | **归档候选**，优先项目内历史，保留证据与替代关系；移删另获授权 |

---

## 6. 链接规范

- 每个知识页应有有意义的关联；专题入口确保可达，不为凑2条链接制造无关引用。示例代码不作真实链接。
...
- `index.md` — 按任务/主题组织的首页，只放入口。新增/重命名/归档页更新最近的专题README；新增主题才改首页。
- `log.md` — 获授权写操作的append-only记录；只读审计报告存任务目录，不因lint改库。
...
2026-09-09重组：根目录保留00/05/10/20/40/50/90及assets。详见 [[90-治理/目录路由与写入规范]]。历史04、10-Topics、30及学习根不再新写。教程编号、标题锚点、研究原始日期保留；不把目录移动当成事实重核。
