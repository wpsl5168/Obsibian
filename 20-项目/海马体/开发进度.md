...
> 更新：2026-04-20 | 基线：PRD v2.1 | 版本：**v0.3.0**（架构审查后）

---

## 最近更新（2026-04-20 架构审查 v0.2 → v0.3）

✅ **v0.3.0 发布** —— 详见 [[审查日志-v0.3-2026-04-20]]
- P0-1 多租户 schema 预留（agent_id/scope/session_id）
- P0-2 SQLite 线程安全（thread-local 连接池）
- P0-3 去重健壮性（embed 失败告警 + hot 语义去重可选）
- P0-4 content_hash O(1) 精确去重
- P1-5 lifespan 替换 on_event
- P1-8 Migration 框架上线（migrations/ 目录）
- 🐛 修复 vec_search 空表 `unknown error`（19 个测试 fail 真因）
- ✅ 测试 36/55 → **78/78 全绿**
...
| 部分完成 | 3个 🔶 |
| 未开始 | 13个 ⬜ |
| 源码文件 | 11个 Python模块（+migrations_runner） |
| 测试用例 | 78个（全通过 ✅） |
| Git commits | 10个 |
